...
#### udev
udev는 커널이 새 장치를 인식하면 이를 처리하여,   
드라이버를 로드하고 장치 파일에 사용자 규칙을 적용하는 데몬이다.
...
1. 부팅 시 udevd 실행
2. udevd가 규칙 파일을 읽어 자신의 메모리 공간에 로드
3. 커널이 장치를 감지하면 sysfs(/sys)에 정보 생성
4. 커널이 uevent를 netlink 소켓을 통해 udevd에게 전달
5. udevd는 규칙에 따라 필요한 경우 드라이버 모듈을 로드
6. 드라이버가 devtmpfs를 통해 장치를 등록하면 커널이 /dev 아래에 기본 장치 파일 생성
7. udevd는 sysfs에서 장치 정보를 확인하고 규칙과 매칭하여 장치 정책 적용  
   - 이름 변경  
   - 권한/소유권 설정  
   - 심볼릭 링크 생성  
   - 사용자 스크립트 실행
...
반면 modprobe는 모듈 이름(또는 alias) 를 기반으로 적재하며,   
필요한 의존 모듈도 함께 로드한다.
