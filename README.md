...
rcu는 버퍼를 동적 할당해야 하기 때문에 적합하지 않다.
...
#### 구현 내용
procfs는 커널 내부 정보를 사용자 공간에 보여주기 위한 파일시스템이다.   
예를 들어 /proc/meminfo는 메모리 사용량을, /proc/cpuinfo는 CPU 정보를 출력한다.   
...
#### nsproxy와 마운트 정보
현재 실행 중인 태스크의 PCB는 current 전역 포인터로 접근할 수 있다.   
이 구조체 안에는 nsproxy라는 필드가 있으며,   
여기에 해당 프로세스가 속한 네임스페이스 정보가 저장되어 있다.
